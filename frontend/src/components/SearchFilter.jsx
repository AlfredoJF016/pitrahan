...
export default function SearchFilter({ bikes = [], onSelectBike }) {
  const [selectedTab, setSelectedTab] = useState('semua');
  const [searchQuery, setSearchQuery] = useState('');
  const [maxPrice, setMaxPrice] = useState(150000); // Default max Rp150.000 per hari
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBikes.map(bike => {
            ...
            if (bike.status_ketersediaan === 'disewa') {
              statusText = 'Disewa';
              badgeClass = 'bg-[#FFD700]/10 text-[#FFD700] border-[#FFD700]/30'; // Kuning
            } else if (bike.status_ketersediaan === 'servis') {
              statusText = 'Servis';
              badgeClass = 'bg-[#FF3E3E]/10 text-[#FF3E3E] border-[#FF3E3E]/30'; // Merah
            }

            return (
              <div 
                key={bike.id} 
                className="bento-card p-5 flex flex-col justify-between space-y-4 hover:border-glow-green"
              >
                {/* Visual Header / Card Image Substitute */}
                <div className="h-44 w-full bg-gradient-to-br from-[#252525] to-[#121212] rounded-xl flex items-center justify-center relative overflow-hidden border border-[#2d2d2d]">
                  {/* Decorative element resembling bicycle wheels or frames */}
                  ...
                  {bike.foto_url ? (
                    ...
                    <div className="text-center space-y-1">
                      <span className="text-3xl font-extrabold text-[#333333] uppercase select-none tracking-widest block">
                        {bike.jenis_sepeda}
                      </span>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xs space-y-1 text-[#a0a0a0]">
                    <p className="flex items-center gap-1">
                      <span className="text-[#00E5FF] font-semibold">Toko:</span> {bike.nama_toko}
                    </p>
                    <p className="line-clamp-1">
                      <span className="text-[#00E5FF] font-semibold">Alamat:</span> {bike.alamat_toko}
                    </p>
                  </div>
                </div>
